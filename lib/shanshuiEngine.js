...
/* ============================================================
   词表、诗库、把一句话读成画面（离线兜底）
   ============================================================ */
export const DYN_ELEM = ['boat', 'waterfall', 'crane', 'birds', 'whale', 'lanterns', 'kite', 'fireflies', 'koi']
export const ELEM_CN = { boat: '小舟', waterfall: '瀑布', crane: '仙鹤', birds: '飞鸟', whale: '鲸鱼', lanterns: '孔明灯', kite: '风筝', fireflies: '萤火虫', koi: '锦鲤' }
...
const KW = {
  ...
  el: { boat: ['船', '舟', '钓', '渔', '划', '帆', 'boat', 'ship', 'sail'], whale: ['鲸', 'whale'], crane: ['鹤', '仙', 'crane'], birds: ['鸟', '雁', '燕', '鸦', 'bird'], lanterns: ['灯', '孔明', 'lantern'], kite: ['风筝', 'kite'], fireflies: ['萤', 'firefl'], koi: ['鱼', '鲤', 'koi', 'fish'], waterfall: ['瀑', '泉', 'waterfall'] },
...
export function localInterpret(text) {
  ...
  const elCN = els.map(e => ELEM_CN[e]).slice(0, 3).join('、')
  const when = time === 'day' ? '白日' : time === 'night' ? '夜里' : time === 'dusk' ? '黄昏' : '清晨'
  const note = (found.length ? `我在你的话里读到了「${found.slice(0, 4).join('」「')}」，` : '你的话里没有具体的景物，我就照着它的语气，') + `把驻地画成了${SEASON_CN[season]}天${when}的样子${elCN ? '，添上' + elCN : ''}。`
  return {
    title: b.title, poem: b.poem, seal: '驻地', season, time, weather,
...
export function normalizeScene(o, text) {
  ...
  let els = Array.isArray(o.elements) ? [...new Set(o.elements.filter(e => DYN_ELEM.includes(e)))].slice(0, 6) : []
...
export function createShanshui(canvas, opts = {}) {
  ...
  const BUILD = {
    ...
    cushion(g, x, y, s, rng, col, fill) { // 蒲团：松下亭
      ...
      drawPine(g, x - s * 1.0, y + 4, s * 2.8, rng, col, fill)
