...
/**
 * 直传 R2:文件不经过 Vercel,因此不受 4.5MB 请求体上限约束。
 * 先向 /api/upload-url 换一个短时有效的上传地址,再把文件直接 PUT 到 R2。
 * 用于:动效视频、超大图等。
 *
 * @param {File} file
 * @param {string} folder
 ...
 * @returns {Promise<{url: string}>}
 */
export async function uploadDirect(file, folder = 'uploads', onProgress) {
  if (!file) throw new Error('未选择文件')
  ...
  if (!signRes.ok) {
    let msg = `上传准备失败(HTTP ${signRes.status})`
    try {
      const err = await signRes.json()
      if (err && err.error) msg = err.error
    } catch (e) { /* 响应不是 JSON,保留默认提示 */ }
    throw new Error(msg)
  }

  const { uploadUrl, publicUrl } = await signRes.json()

  // 2. 直接 PUT 到 R2。用 XMLHttpRequest 是为了拿到上传进度
