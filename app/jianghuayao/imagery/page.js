...
export default function ImageryPage() {
  ...
  const tabArticles = articles.filter(a => a.type === 'imagery' && a.relatedType === activeTab);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="relative h-48 md:h-[250px] overflow-hidden flex items-center bg-cover bg-center"
        style={{ backgroundImage: 'url("https://tr-osdcp.qunarzz.com/tr-osd-tr-space/img/f23fb894148dddadee9abd69e862c928.jpg_r_1360x1360x95_035a2e8d.jpg")' }}>
      ...
      <div className="py-12 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
            ...
            {IMAGERY_TYPES.map(type => (
              <button key={type.id} onClick={() => setActiveTab(type.id)}
                className={`flex items-center gap-2 px-6 py-3 rounded-lg font-medium transition ${
                  activeTab === type.id ? 'bg-purple-500 text-white' : 'bg-white text-gray-700 border-2 border-gray-200 hover:border-purple-500'
                }`}>
          ...
          <div>
            <h2 className="text-3xl font-bold text-gray-800 mb-8 text-center">{currentTab?.name}</h2>
            {tabArticles.length === 0 ? (
              <div className="text-center py-12 text-gray-500"><p>暂无内容</p></div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {tabArticles.map(article => <div key={article.id}><ArticleCard article={article} /></div>)}
              </div>
            )}
          </div>
        </div>
