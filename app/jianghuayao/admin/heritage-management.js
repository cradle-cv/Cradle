...
export default function HeritageManagement() {
  ...
  const handleUpdateProject = () => {
    const updated = projects.map(p => 
      p.id === editingId 
        ? { ...p, ...formData, categoryName: CATEGORIES.find(c => c.id === formData.category)?.name || '' }
        : p
    );
    setProjects(updated);
    saveToStorage(updated);
  ...
  const resetForm = () => {
    setFormData({
      title: '', category: 'folk-customs', description: '', details: '',
      year: new Date().getFullYear().toString(), area: '江华县', image: '',
    });
    setEditingId(null);
    setShowForm(false);
  };
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h3 className="text-xl font-bold mb-4">{editingId ? '编辑项目' : '新增项目'}</h3>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">项目名称 *</label>
              <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="输入项目名称" />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="输入简短描述" rows="3" />
            </div>
            <div>
              ...
              <textarea value={formData.details} onChange={(e) => setFormData({ ...formData, details: e.target.value })}
                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="输入详细描述" rows="4" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" value={formData.area} onChange={(e) => setFormData({ ...formData, area: e.target.value })}
                  className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="江华县" />
              </div>
            </div>
            ...
            <div>
              ...
              <ImageUploader
                ...
                folder="jianghuayao/heritage"
              />
            </div>

            <div className="flex gap-3 pt-4">
              <button onClick={editingId ? handleUpdateProject : handleAddProject}
            ...
            {projects.map(project => (
              <tr key={project.id} className="border-b hover:bg-gray-50">
                ...
                <td className="py-3 px-4">
                  <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm">{project.categoryName}</span>
                </td>
                <td className="py-3 px-4">{project.year}</td>
                <td className="py-3 px-4 flex gap-2">
                  <button onClick={() => handleEditProject(project)} className="text-blue-600 hover:text-blue-700 transition" title="编辑"><Edit2 size={18} /></button>
                  <button onClick={() => handleDeleteProject(project.id)} className="text-red-600 hover:text-red-700 transition" title="删除"><Trash2 size={18} /></button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
        {projects.length === 0 && <div className="p-8 text-center text-gray-500">暂无项目，点击"新增项目"开始添加</div>}
      </div>

      <div className="mt-6 bg-blue-50 rounded-lg p-4">
        <p className="text-sm text-gray-700">总共 <strong>{projects.length}</strong> 个项目</p>
      </div>
    </div>
  );
}