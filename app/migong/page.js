...
import { sb, genCode, fmtDate, Shell, Toast, useToast, BASE_PATH } from "./shared"
...
export default function MigongHost() {
  ...
  async function create() {
    ...
    const { error } = await sb.from("migong_rounds").insert({
      code, seed, size, title: title.trim() || null, status: "open",
    })
    setBusy(false)
      ...
      <div className="mg-wrap">
        ...
        <div className="mg-panel">
          ...
          <div className="mg-row" style={{ marginBottom: 14 }}>
            <span className="mg-muted">难度</span>
                ...
                <div key={r.id} className="mg-item">
                  <div className="name">
                    ...
                    <div className="mg-muted" style={{ fontSize: 12, marginTop: 2 }}>
                      {r.code} · {r.size}×{r.size} · {fmtDate(r.created_at)} · {r.status === "open" ? "进行中" : "已结束"}
                    </div>
                  </div>
