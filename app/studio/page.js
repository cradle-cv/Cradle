...
export default function StudioPage() {
  ...
  async function loadData() {
    try {
      ...
      // 未达Lv6直接返回
      if ((userData.level || 1) < 6) { setLoading(false); return }
      ...
      // 加载展览邀请（用户关联的展览）
      if (artist) {
        const { data: exWorks } = await supabase
          .from('exhibition_artworks')
          .select('exhibition_id, exhibitions(*)')
          .eq('artwork_id', artist.id)
        // 简单获取活跃展览
          ...
          <>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <EquippedBadges userId={user.id} size="xs" />
                    {isVerified && (
            ...
            {!isVerified && !isPending && (
              <div className="bg-white rounded-2xl p-8 shadow-sm text-center">
                ...
                <h3 className="text-xl font-bold mb-2" style={{ color: '#111827' }}>开始你的创作之旅</h3>
                <p className="text-sm mb-6" style={{ color: '#6B7280' }}>
                  上传你的第一幅作品，然后申请艺术家认证。<br />
                  认证通过后将解锁完整的工作台功能。
                </p>
                <div className="flex justify-center gap-4">
                  <Link href="/admin/artworks/new"
                    className="px-6 py-3 rounded-xl text-sm font-medium text-white" style={{ backgroundColor: '#111827' }}>
                    上传作品
                  </Link>
                  <Link href="/admin/collections/new"
                    className="px-6 py-3 rounded-xl text-sm font-medium border" style={{ color: '#374151', borderColor: '#D1D5DB' }}>
                    创建作品集
                  </Link>
                </div>
              </div>
