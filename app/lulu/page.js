...
const delGroup=i=>setGroups(p=>p.filter((*,idx)=>idx!==i).map((g,j)=>({…g,seq:j+1})))
...
const delQ=i=>{setQs(p=>p.filter((*,idx)=>idx!==i).map((q,j)=>({…q,seq:j+1})));setEditQ(null)}
...
1. 使用语义化标签：h1、h2、p、strong、table 等
1. 为每个 H2 标题加不同的 inline color 样式（如 color:#2563eb）
1. 表格使用 border=“1” style=“border-collapse:collapse;width:100%”，td/th 用 padding:8px 12px
1. 加粗使用 <strong> 标签
1. 不要添加任何 CSS class，只用 inline style
1. 直接返回 HTML，不要任何解释文字`}]
   })
   })
...
// Editor CSS
const docCSS=`.te-paper h1{font-size:22px;font-weight:900;color:#111827;line-height:1.3;margin:0 0 10px;font-family:Georgia,serif;} .te-paper h2{font-size:17px;font-weight:800;margin:20px 0 6px;font-family:Georgia,serif;} .te-paper p{margin:0 0 12px;color:#374151;font-family:Georgia,serif;} .te-paper table{width:100%;border-collapse:collapse;margin:16px 0;font-size:13px;} .te-paper td,.te-paper th{border:1px solid #cbd5e1;padding:8px 12px;text-align:left;} .te-paper th{background:#f0f4f8;font-weight:700;} .te-paper strong,.te-paper b{font-weight:700;}`
...
if(sessionId){
// Insert a separate row for Excel phase (different phase tag)
await sb.from(“word_lab_submissions”).insert({
session_id:sessionId,student_name:studentName,
score:s.total,max_score:80,submitted:true,phase:‘excel’,
...
<tbody>
{Array.from({length:dataRows},(*,rowIdx)=>{
const r=rowIdx+1
return(
<tr key={r}>
...
const docCSS = `.doc-paper h1{font-size:22px;font-weight:900;color:#111827;line-height:1.3;margin:0 0 10px;font-family:Georgia,serif;} .doc-paper h2{font-size:17px;font-weight:800;margin:20px 0 6px;font-family:Georgia,serif;} .doc-paper p{margin:0 0 12px;color:#374151;font-family:Georgia,serif;} .doc-paper table{width:100%;border-collapse:collapse;margin:16px 0;font-size:13px;} .doc-paper td,.doc-paper th{border:1px solid #cbd5e1;padding:8px 12px;text-align:left;} .doc-paper th{background:#f0f4f8;font-weight:700;} .doc-paper tr:nth-child(even) td{background:#f8fafc;} .doc-paper strong,.doc-paper b{font-weight:700;}`
