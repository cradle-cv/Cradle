...
export default function ArtworkDetailPage() {
  ...
  const [loadError, setLoadError] = useState('')  // 诊断用:记录加载失败原因,先不跳首页
  ...
  async function loadData() {
    try {
      ...
      if (wErr) {
        console.error('加载作品出错:', wErr)
        setLoadError(`查询报错:${wErr.message || JSON.stringify(wErr)}（code: ${wErr.code || '无'}）`)
        setLoading(false)
        return
      }
      if (!w) {
        console.warn('作品查询返回空, id =', id)
        setLoadError(`作品查询返回空。id = ${id}。最可能是 artworks 表的 RLS 策略未放行公开读取。`)
        setLoading(false)
        return
      }
      setArtwork(w)
  ...
  // 诊断:加载失败时显示原因,不再静默跳首页
  if (loadError) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-6">
        <div className="max-w-lg bg-white rounded-2xl shadow-sm p-8 text-center">
          <div className="text-4xl mb-4">🔍</div>
          <h1 className="text-lg font-bold mb-3" style={{ color: '#111827' }}>作品加载失败</h1>
          <p className="text-sm mb-6" style={{ color: '#6B7280', lineHeight: 1.8, wordBreak: 'break-word' }}>
            {loadError}
          </p>
          <Link href="/" className="inline-block px-5 py-2.5 rounded-lg text-sm font-medium text-white" style={{ backgroundColor: '#111827' }}>
            返回首页
          </Link>
        </div>
      </div>
