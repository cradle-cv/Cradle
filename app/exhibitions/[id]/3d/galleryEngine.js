// ================================================================
// 摇篮 · 3D 展厅引擎
// 路径: app/exhibitions/[id]/3d/galleryEngine.js
//
// 纯 three.js，不依赖 React。Exhibition3DClient.js 负责界面与数据。
//
// 这一版的要点：
...
const THEMES = {
  whitebox: {
    bg: 0xefede8, wall: 0xe9e6e0, floor: 'concrete', ceil: 0xf4f3f0, base: 0xd2cdc3,
    frame: { color: 0x1d1d1d, metal: 0.1, rough: 0.55, width: 0.035, depth: 0.045, bevel: 0.004 },
    pool: 0xfff1d8, poolStrength: 0.2, shadow: 0.30, envI: 1.0, exposure: 1.0, hemi: 0.35,
    label: { bg: '#fbfaf7', fg: '#222', sub: '#777', line: '#c9c4b8' }, title: '#2a2a2a', fog: null,
    skylights: true,
  },
  classic: {
    bg: 0x14151c, wall: 0x2a2c38, floor: 'wood', ceil: 0x16171e, base: 0x1a1b22,
    frame: { color: 0xb8914f, metal: 0.85, rough: 0.32, width: 0.07, depth: 0.06, bevel: 0.012 },
    pool: 0xffe2b0, poolStrength: 0.8, shadow: 0.5, envI: 0.38, exposure: 1.05, hemi: 0.12,
    label: { bg: '#1c1d26', fg: '#f2ead9', sub: '#b8a57f', line: '#c9a96e' }, title: '#e8d6b0', fog: 0.018,
    skylights: false,
  },
}
THEMES.lshape = THEMES.classic
THEMES.circular = { ...THEMES.classic, wall: 0x262734, bg: 0x101117 }
...
function makeFloorTexture(kind, maxAniso) {
  ...
  if (kind === 'wood') {
    ...
    for (let r = 0; r < rows; r++) {
      let x = -((r * 373) % 900)
      while (x < 1024) {
        const len = 500 + ((r * 97 + x) % 400)
        const v = 40 + ((r * 31 + Math.abs(x) * 7) % 22)
        g.fillStyle = `rgb(${v + 18},${v + 8},${v})`
        g.fillRect(x, r * h, len, h)
        for (let k = 0; k < 18; k++) {
          g.strokeStyle = `rgba(0,0,0,${0.04 + (k % 3) * 0.02})`
          g.beginPath(); const yy = r * h + (k * 13.7) % h
          g.moveTo(x, yy); g.bezierCurveTo(x + len * 0.3, yy + 3, x + len * 0.6, yy - 3, x + len, yy); g.stroke()
        }
        g.fillStyle = 'rgba(0,0,0,0.45)'; g.fillRect(x, r * h, 2, h)
        x += len
      }
      g.fillStyle = 'rgba(0,0,0,0.5)'; g.fillRect(0, r * h, 1024, 2)
    }
  } else {
    // 抛光水泥：底色 + 细噪点 + 几道淡淡的伸缩缝
    g.fillStyle = '#d6d2ca'; g.fillRect(0, 0, 1024, 1024)
    ...
    for (let k = 0; k < 40; k++) {
      g.fillStyle = `rgba(${Math.random() < 0.5 ? '0,0,0' : '255,255,255'},0.025)`
      g.beginPath(); g.arc(Math.random() * 1024, Math.random() * 1024, 60 + Math.random() * 180, 0, Math.PI * 2); g.fill()
    }
...
// 创建展厅
// ================================================================
export function createGallery({ container, exhibition, works, images, isMobile, onFocus, onReady }) {
  const style = ['whitebox', 'classic', 'lshape', 'circular'].includes(exhibition?.gallery_style) ? exhibition.gallery_style : 'classic'
  ...
  // ---------- 平面图 ----------
  let plan
  const explicit = works.some(w => (w.wall_position || 0) > 0)
  if (style === 'lshape') plan = planL(items)
  else if (style === 'circular') plan = planRotunda(items)
  ...
  const H = style === 'whitebox' ? 5.2 : style === 'circular' ? Math.min(8, 5 + (plan.R || 0) * 0.12) : 4.6
  ...
  const floorMat = track(new THREE.MeshStandardMaterial({ map: floorTex, roughness: th.floor === 'wood' ? 0.42 : 0.5, metalness: 0, envMapIntensity: 1.2 }))
  const ceilMat = track(new THREE.MeshStandardMaterial({ color: th.ceil, roughness: 1, emissive: th.ceil, emissiveIntensity: style === 'whitebox' ? 0.45 : 0.15 }))
  ...
  const fixtureMat = track(new THREE.MeshStandardMaterial({ color: style === 'whitebox' ? 0xe9e9e9 : 0x111111, metalness: 0.6, roughness: 0.35 }))
  ...
  floorTex.repeat.set(fw / (th.floor === 'wood' ? 3 : 8), fd / (th.floor === 'wood' ? 3 : 8))
  ...
  if (th.skylights) {
    // 白盒子：天花上的条形灯膜
    const skyMat = track(new THREE.MeshBasicMaterial({ color: 0xffffff, toneMapped: false }))
  ...
  // ---------- 长凳 ----------
  const benchMat = track(new THREE.MeshStandardMaterial({ color: style === 'whitebox' ? 0xb89a74 : 0x3b2f27, roughness: 0.55 }))
  ...
  order.forEach((p, orderIdx) => {
    ...
    if (item.img) {
      ...
      mat = track(new THREE.MeshBasicMaterial({ map: tex, toneMapped: false, color: new THREE.Color(style === 'whitebox' ? 0.97 : 0.93, style === 'whitebox' ? 0.97 : 0.92, style === 'whitebox' ? 0.96 : 0.9) }))
    } else {
  ...
  // 地面落点提示圈
  const ringMat = track(new THREE.MeshBasicMaterial({ color: style === 'whitebox' ? 0x333333 : 0xffffff, transparent: true, opacity: 0.35, depthWrite: false }))
  ...
  function drawMap() {
    ...
    infos.forEach((f, i) => {
      ...
      ctx.beginPath(); ctx.arc(X(f.x), Z(f.z), i === focused ? 3.2 : 2, 0, Math.PI * 2); ctx.fill()
    })
