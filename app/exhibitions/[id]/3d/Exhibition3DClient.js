...
export default function Exhibition3DPage() {
  ...
  async function loadData() {
    try {
      ...
      if (ex) { setExhibition(ex); styleRef.current = ex.gallery_style || 'classic' }
      ...
      const works = exArtworks?.map(ea => ({
        ...ea.artworks, wall_side: ea.wall_side || 'left', wall_position: ea.wall_position || 0
      })).filter(a => a && a.id) || []
  ...
  function buildClassic(scene, camera, tl, leftW, rightW) {
    ...
    addPlane(scene,rW,rL,fM,[0,0,0],[-Math.PI/2,0,0])
    addPlane(scene,rW,rL,new THREE.MeshStandardMaterial({color:0x222233}),[0,rH,0],[Math.PI/2,0,0])
    addPlane(scene,rL,rH,wM,[-rW/2,rH/2,0],[0,Math.PI/2,0])
    addPlane(scene,rL,rH,wM,[rW/2,rH/2,0],[0,-Math.PI/2,0])
    ...
    for(let z=-rL/2+6;z<rL/2-3;z+=sp*2) addBench(scene,[0,0,z],0x3a3a4a)
  }
  ...
  function buildWhitebox(scene, camera, tl, leftW, rightW) {
    ...
    addPlane(scene,rW,rL,fM,[0,0,0],[-Math.PI/2,0,0])
    addPlane(scene,rW,rL,new THREE.MeshStandardMaterial({color:0xffffff}),[0,rH,0],[Math.PI/2,0,0])
    addPlane(scene,rL,rH,wM,[-rW/2,rH/2,0],[0,Math.PI/2,0])
    addPlane(scene,rL,rH,wM,[rW/2,rH/2,0],[0,-Math.PI/2,0])
    addPlane(scene,rW,rH,wM,[0,rH/2,-rL/2],[0,0,0])
    addPlane(scene,rW,rH,wM,[0,rH/2,rL/2],[0,Math.PI,0])
    ...
    for(let z=-rL/2+8;z<rL/2-3;z+=sp*2) addBench(scene,[0,0,z],0xcccccc)
  }
      ...
      {!started&&(
        <div className="absolute inset-0 z-20 flex items-center justify-center" style={{background:'linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%)'}}>
          <div className="text-center max-w-lg px-8">
            ...
            <button onClick={startExperience} className="mt-6 px-10 py-4 rounded-2xl text-lg font-bold text-white hover:scale-105 active:scale-95 transition-all" style={{background:'linear-gradient(135deg,#c9a96e,#b08d4f)',boxShadow:'0 8px 30px rgba(201,169,110,0.3)'}}>进入展厅</button>
      ...
      {started&&(
        <>
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 pointer-events-none">
            <div className="w-6 h-6 border-2 border-white/30 rounded-full flex items-center justify-center"><div className="w-1 h-1 bg-white/50 rounded-full"/></div>
          </div>
          <div className="absolute top-4 left-4 z-10">
            <Link href={`/exhibitions/${id}`} className="px-4 py-2 rounded-lg text-sm text-white/60 hover:text-white hover:bg-white/10">← 返回展览</Link>
          </div>
          <div className="absolute top-4 right-4 z-10 text-right">
            <p className="text-white/40 text-xs">{exhibition?.title||'每日一展'}</p>
            <p className="text-white/20 text-xs">{artworks.length} 件 · {sn[exhibition?.gallery_style]||'经典长廊'}</p>
          </div>
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-10"><p className="text-white/20 text-xs">走近画作并点击查看详情</p></div>
        </>
      )}
      ...
      {viewingArtwork&&(
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={()=>setViewingArtwork(null)}>
          <div className="bg-[#1e1e30] rounded-2xl overflow-hidden max-w-2xl w-full mx-4 shadow-2xl border border-white/10" onClick={e=>e.stopPropagation()}>
            {viewingArtwork.image_url&&<div className="w-full" style={{maxHeight:'50vh',overflow:'hidden'}}><img src={viewingArtwork.image_url} alt={viewingArtwork.title} className="w-full h-auto object-contain" style={{maxHeight:'50vh'}}/></div>}
            <div className="p-6">
